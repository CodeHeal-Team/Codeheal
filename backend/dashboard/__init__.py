...
# â”€â”€ make sure the codeheal package root is on sys.path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
_BUNDLED_SCENARIOS: dict[str, str] = {
    "none_bug   â€” None-guard missing in divide()": str(
        _SAMPLES_DIR / "none_bug"
    ),
    "off_by_one â€” Off-by-one in last_element()": str(
        _SAMPLES_DIR / "off_by_one"
    ),
    "broken_api â€” Wrong HTTP method in router": str(
...
# Status â†’ (emoji, CSS colour class name used in styling)
_STATUS_STYLE: dict[str, tuple[str, str]] = {
    "pending":     ("â¬œ", "#e5e7eb"),
    "in_progress": ("ðŸ”§", "#fbbf24"),
    "done":        ("âœ…", "#22c55e"),
    "failed":      ("âŒ", "#ef4444"),
}
...
def _render_timeline(timeline: list[TimelineEvent]) -> None:
    ...
    # Build a dict of label â†’ latest status for quick lookup
    latest: dict[str, str] = {}
...
def _render_diagnosis(state: PipelineState) -> None:
    ...
    if d.affected_file:
        lines_str = (
            ", ".join(str(ln) for ln in d.affected_lines) if d.affected_lines else "â€”"
        )
        st.markdown(f"**Affected File:** `{d.affected_file}` â€” lines {lines_str}")
...
def _render_diff(state: PipelineState) -> None:
    ...
    st.subheader("ðŸ“„ Side-by-Side Diff")
...
def _render_generated_test(state: PipelineState) -> None:
    ...
    st.subheader("ðŸ§ª Generated Test")

    status_label = (
        "âœ… Confirmed failing (bug verified)" if gt.confirmed_failing
        else "âš ï¸ Could not confirm failure"
    )
...
def _render_terminal_panels(state: PipelineState) -> None:
    ...
    if state.initial_test_result.stdout or state.initial_test_result.stderr:
        with st.expander("Terminal â€” Initial Test Run", expanded=False):
    ...
    if state.final_test_result.stdout or state.final_test_result.stderr:
        with st.expander("Terminal â€” Final Test Run (after fix)", expanded=False):
...
def _render_metrics(state: PipelineState) -> None:
    ...
    before_label = "âŒ Failing" if not state.initial_test_result.passed else "âœ… Passing"
    after_label = "âœ… Passing" if state.final_test_result.passed else "âŒ Failing"
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("â–¶ Run CodeHeal", disabled=run_disabled, type="primary", use_container_width=True):
        ...
        if st.button("ðŸ”„ Reset Repository", use_container_width=True):
            reset_repo(repo_path)
            st.session_state["pipeline_state"] = None
            st.session_state["pipeline_running"] = False
            st.rerun()
    ...
    st.title("ðŸ©º CodeHeal â€” Automated Bug Repair")
    ...
    if state is not None and state.stage == PipelineStage.FAILED:
        st.error(f"**Pipeline Failed** â€” {state.error or 'Unknown error'}", icon="âŒ")

    # Status bar
    if running:
        st.info("â³ Pipeline is runningâ€¦ Page refreshes automatically.", icon="ðŸ”§")
    elif state is None:
        st.info(
            "Select a scenario in the sidebar and click **â–¶ Run CodeHeal** to start.",
            icon="ðŸ‘ˆ",
        )

...
Test strategy
~~~~~~~~~~~~~
1. ``_resolve_model_id`` â€” startup model availability check and fallback.
2. ``WatsonxAgent.__init__`` â€” credential validation, client construction.
3. ``WatsonxAgent.generate`` â€” response parsing, error handling.
4. Each agent's ``_build_prompt`` â€” content inspection without touching the model.
5. Each agent's ``run`` â€” verifies the full generateâ†’return pipeline with a mocked
   ``ModelInference.generate`` response.
6. Integration with ``response_parser`` â€” raw mocked output parses cleanly.
"""
...
# ---------------------------------------------------------------------------
# _resolve_model_id â€” model availability check
# ---------------------------------------------------------------------------
...
class TestResolveModelId:
    ...
    def test_falls_back_to_first_granite_instruct(self):
        ...
        mock_specs = {
            "resources": [
                {"model_id": "ibm/granite-13b-instruct-v2"},
                {"model_id": "ibm/granite-3-3-8b-chat"},  # not instruct â€” should not match
...
# ---------------------------------------------------------------------------
# WatsonxAgent.generate â€” response parsing
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Response parser integration â€” confidence clamping and retry
# ---------------------------------------------------------------------------
